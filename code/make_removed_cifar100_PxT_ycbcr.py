...
if __name__ == "__main__":
    for QF in QFs:
        ...
        model.load_state_dict(
            torch.load(f"./models/PxT_v2_cifar100_ycbcr_5.pth", map_location=device)
        )
        ...
        with torch.no_grad():
            ...
            for input_images, target_images in tqdm.tqdm(
                ...
                for i in range(len(outputs)):
                    ...
                    np.clip(ycrcb_target, 0, 1, out=ycrcb_target)
                    np.clip(ycrcb_output, 0, 1, out=ycrcb_output)
                    ycrcb_target = np.transpose(ycrcb_target, (1, 2, 0))
                    ycrcb_output = np.transpose(ycrcb_output, (1, 2, 0))
                    ycrcb_target = (ycrcb_target * 255).astype(np.uint8)
                    ycrcb_output = (ycrcb_output * 255).astype(np.uint8)
                    ...
                    # sys.exit(1)
                    bgr_target_patches.append(bgr_target)
                    bgr_output_patches.append(bgr_output)
                    patch_idx += 1

                    # 8x8 이미지들을 32x32로 합치기
                    if patch_idx % 16 == 0 and patch_idx > 0:
                        patch_idx = 0
                        combined_target_image = np.zeros((32, 32, 3), dtype=np.float32)
                        combined_output_image = np.zeros((32, 32, 3), dtype=np.float32)
                        ...
                        for j in range(4):
                            for k in range(4):
                                ...
                                combined_target_image[
                                    j * 8 : (j + 1) * 8, k * 8 : (k + 1) * 8, :
                                ] = bgr_target_patches[idx]
                        bgr_target_patches.clear()
                        ...
                        for j in range(4):
                            for k in range(4):
                                ...
                                combined_output_image[
                                    j * 8 : (j + 1) * 8, k * 8 : (k + 1) * 8, :
                                ] = bgr_output_patches[idx]

                        bgr_output_patches.clear()
                        ...
                        combined_target_image_path = os.path.join(
                            ...
                            "test",
                            f"{class_idx:03d}",
                            f"combined_target{image_name_idx}.png",
                        )
