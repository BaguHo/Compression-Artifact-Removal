...
def load_images(QF):
    ...
    target_train_dataset_dir = os.path.join(mini_imagenet_path, "_original", "train")
...
if __name__ == "__main__":
    for QF in QFs:
        ...
        for i in range(num_classes):
            train_class_length.append(
                len(os.listdir(f"datasets/mini-imagenet/jpeg{QF}/train/{i}"))
            )
            test_class_length.append(
                len(os.listdir(f"datasets/mini-imagenet/jpeg{QF}/test/{i}"))
            )
        print(f"Train class length: {len(train_class_length)}")
        print(f"Test class length: {len(test_class_length)}")
        # Load the dataset
        ...
        for model_name in model_names:
            ...
            with torch.no_grad():
                ...
                for input_images, target_images in tqdm.tqdm(
                    train_loader, desc=f"Making Train Images jpeg{QF}"
                ):
                    ...
                    for i in range(len(outputs)):
                        ...
                        os.makedirs(f"datasets/{model_name}_mini_imagenet/jpeg{QF}/train/{class_idx}", exist_ok=True)
                        ...
                        idx += 1
                        if idx == train_class_length[class_idx] and class_idx + 1 != num_classes:
                            idx = 0
                            class_idx += 1
                            # print(f"current class: {class_idx}/{num_classes}")

                idx = 0
                class_idx = 0
                for input_images, target_images in tqdm.tqdm(
                    test_loader, desc=f"Making Test Images jpeg{QF}"
                ):
                    input_images = input_images.to(device)
                    target_images = target_images.to(device)
                    ...
                    for i in range(len(outputs)):
                        ...
                        lpips_alex = lpips_alex_model(
                            target_images[i].to(device), outputs[i].to(device), normalize=True
                        ).item()
                        ...
                        os.makedirs(f"datasets/{model_name}_mini_imagenet/jpeg{QF}/test/{class_idx}", exist_ok=True)
                        output_image_path = os.path.join(
                            "datasets",
                            f"{model_name}_mini_imagenet",
                            f"jpeg{QF}",
                        ...
                        cv2.imwrite(output_image_path, output_image)
                        idx += 1
                        if idx == test_class_length[class_idx] and class_idx + 1 != num_classes:
                            idx = 0
                            class_idx += 1
            ...
            print(
                f"Model: {model_name}, QF: {QF}, Average PSNR: {avg_psnr:.2f} dB, Average SSIM: {np.mean(ssim_values):.4f}, Average LPIPS Alex: {avg_lpips_alex:.4f}"
            )
            logging.info(
                f"Model: {model_name}, QF: {QF}, Average PSNR: {avg_psnr:.2f} dB, Average SSIM: {np.mean(ssim_values):.4f}, Average LPIPS Alex: {avg_lpips_alex:.4f}"
            )
