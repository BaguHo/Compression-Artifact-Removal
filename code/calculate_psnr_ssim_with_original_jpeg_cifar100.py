...
def calculate_psnr_ssim_lpips(QF):
    ...
    print(f"CIFAR100 original-jpeg QF {QF}: PSNR = {np.mean(psnr_values):.2f}, SSIM = {np.mean(ssim_values):.4f}, LPIPS = {np.mean(lpips_values):.4f}")
    logging.info(f"CIFAR100 original-jpeg QF {QF}: PSNR = {np.mean(psnr_values):.2f}, SSIM = {np.mean(ssim_values):.4f}, LPIPS = {np.mean(lpips_values):.4f}")
