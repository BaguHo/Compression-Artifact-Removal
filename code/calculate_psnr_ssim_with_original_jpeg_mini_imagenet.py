...
def load_mini_imagenet_test_dataset_and_dataloader(QF):
    ...
    # 테스트 데이터 로드
    for i in tqdm.tqdm(range(1000), desc=f"Loading test data (QF {QF})"):
...
def calculate_psnr_ssim_lpips(QF):
    ...
    test_dataset, test_loader = load_mini_imagenet_test_dataset_and_dataloader(QF)
    ...
    print(f"QF {QF}: PSNR = {np.mean(psnr_values):.2f}, SSIM = {np.mean(ssim_values):.4f}, LPIPS = {np.mean(lpips_values):.4f}")
    logging.info(f"QF {QF}: PSNR = {np.mean(psnr_values):.2f}, SSIM = {np.mean(ssim_values):.4f}, LPIPS = {np.mean(lpips_values):.4f}")
