...
if __name__ == "__main__":
    for QF in QFs:
        ...
        model.load_state_dict(
            torch.load(f"./models/PxT_32_32_patch_size_8_20.pth", map_location=device)
        )
        ...
        with torch.no_grad():
            ...
            for input_images, target_images in tqdm.tqdm(
                ...
                for i in range(len(outputs)):
                    ...
                    np.clip(rgb_target, 0, 1, out=rgb_target)
                    np.clip(rgb_output, 0, 1, out=rgb_output)
                    # rgb_target = np.transpose(rgb_target, (1, 2, 0))
                    # rgb_output = np.transpose(rgb_output, (1, 2, 0))
                    rgb_target_patches.append(rgb_target)
                    rgb_output_patches.append(rgb_output)
                    patch_idx += 1
        ...
        print(
            f"PxT_32_32_patch_size_8, QF: {QF},Test Loss: {avg_test_loss:.4f}, PSNR: {avg_psnr:.2f} dB, SSIM: {avg_ssim:.4f}, LPIPS: {avg_lpips:.4f}"
        )
        logging.info(
            f"PxT_32_32_patch_size_8, QF: {QF},Test Loss: {avg_test_loss:.4f}, PSNR: {avg_psnr:.2f} dB, SSIM: {avg_ssim:.4f}, LPIPS: {avg_lpips:.4f}"
        )
