    ...
    for model_name in model_names:
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 10 == 0 or (epoch + 1) == epochs:
                torch.save(model.state_dict(),os.path.join("models", f"{model_name}_cifar100_{epoch+1}_20250515.pth"))
        ...
        torch.save(model.state_dict(),os.path.join("models", f"{model_name}_cifar100_final_20250515.pth"))
        print(f"Final model saved as {model_name}_final_20250515.pth")
        logging.info(f"Final model saved as {model_name}_final_20250515.pth")
            ...
            with torch.no_grad():
                for input_images, target_images in tqdm.tqdm(
                    ...
                    for i in range(len(outputs)):
                        ...
                        image_name_idx += 1
                        output_image_dir = os.path.join("datasets", "CIFAR100", f"{model_name}_output_20250515", "test",f"jpeg{QF}",f"{class_idx:03d}")
                        os.makedirs(output_image_dir, exist_ok=True)
                        output_image_path = os.path.join(output_image_dir, f"{image_name_idx:05d}.png")
                        Image.fromarray((output_image * 255).transpose(1, 2, 0).astype(np.uint8)).save(output_image_path)
