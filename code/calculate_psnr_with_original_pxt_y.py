...
def load_cifar100_test_dataset_and_dataloader(QF, dataset_name):
    ...
    # print("test_input_dataset.shape, test_target_dataset.shape", np.array(test_input_dataset).shape, np.array(test_target_dataset).shape)
    # input()
    # Dataset과 DataLoader 생성
    test_dataset = load_dataset.CustomDataset(test_input_dataset, test_target_dataset)
    ...
    with torch.no_grad():
        for input_images, target_images in tqdm.tqdm(test_loader, desc=f"Calculating PSNR and SSIM for QF {QF}"):
            for i in range(len(input_images)):
                ...
                # print("target_image.shape, output_image.shape", target_image.shape, output_image.shape)
                # input()
