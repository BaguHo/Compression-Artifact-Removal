    ...
    for model_name in model_names:
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 10 == 0 or (epoch + 1) == epochs:
                torch.save(
                    model.state_dict(),
                    os.path.join("models", f"{model_name}_cifar100_{epoch+1}.pth"),
                )
        ...
        torch.save(
            model.state_dict(),
            os.path.join("models", f"{model_name}_cifar100_final.pth"),
        )
        ...
        for QF in QFs:
            test_dataset, test_dataloader = (
                load_cifar100_test_dataset_and_dataloader(QF)
            )
            model.eval()
            idx = 0
            test_loss = 0.0
            ...
            save_metrics(
                metrics,
                os.path.join("metrics", f"{model_name}_cifar100_metrics.csv"),
            )
