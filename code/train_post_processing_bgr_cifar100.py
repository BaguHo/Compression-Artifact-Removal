...
if __name__ == "__main__":
    ...
    for model_name in model_names:
        ...
        for QF in QFs:
            ...
            with torch.no_grad():
                for input_images, target_images in tqdm.tqdm(test_dataloader, desc=f"{model_name} Testing QF{QF}"):
                    ...
                    for i in range(len(outputs)):
                        ...
                        bgr_target = target_images[i].cpu().numpy()
                        bgr_output = outputs[i].cpu().numpy()
                        bgr_input = input_images[i].cpu().numpy()
                        # [c,h,w] --> [h,w,c]
                        bgr_target = (bgr_target * 255).astype(np.uint8).transpose(1, 2, 0)
                        bgr_output = (bgr_output * 255).astype(np.uint8).transpose(1, 2, 0)
                        bgr_input = (bgr_input * 255).astype(np.uint8).transpose(1, 2, 0)

                        # Calculate PSNR and SSIM
                        psnr_value = psnr(bgr_target, bgr_output, data_range=255)
                        ssim_value = ssim(bgr_target, bgr_output, data_range=255, channel_axis=2)
                    ...
                    for i in range(len(outputs)):
                        ...
                        cv2.imwrite(os.path.join("datasets", f"{model_name}_ycrcb_cifar100","jpeg{QF}",f"output_{i+1:05d}.png"), bgr_output)
            ...
            print(f"Model: {model_name}_ycrcb_cifar100, Epoch: {epochs}, QF: {QF}, Training Time: {time.ctime(end_time)}, Test Loss: {avg_test_loss:.4f}, Average PSNR: {avg_psnr:.2f} dB, Average SSIM: {avg_ssim:.4f}, Average LPIPS Alex: {avg_lpips_alex:.4f}")
            logging.info(f"Model: {model_name}_ycrcb_cifar100, Epoch: {epochs}, QF: {QF}, Training Time: {time.ctime(end_time)}, Test Loss: {avg_test_loss:.4f}, Average PSNR: {avg_psnr:.2f} dB, Average SSIM: {avg_ssim:.4f}, Average LPIPS Alex: {avg_lpips_alex:.4f}")
