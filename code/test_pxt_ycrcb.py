...
import time
import lpips
from load_dataset import load_test_dataset_and_dataloader_8x8_ycrcb_each_qf
...
if __name__ == "__main__":
    ...
    # load train dataset
    # train_dataset, train_loader = load_dataset_and_dataloader_all_qf(is_train=True,color_space="ycrcb",size="8x8",batch_size=batch_size,num_workers=num_workers)
    ...
    for QF in QFs:
        ...
        with torch.no_grad():
            ycrcb_target_patches = []
            ycrcb_output_patches = []
            ycrcb_input_patches = []
            patch_idx = 0
            class_idx = 0
            image_name_idx = 0
            ...
            for input_images, target_images in tqdm.tqdm(test_loader, desc=f"Testing QF {QF}"):
                ...
                for i in range(len(outputs)):
                    ...
                    # [c,h,w] --> [h,w,c]
                    ycrcb_target = (y_target * 255).astype(np.uint8).transpose(1, 2, 0)
                    ycrcb_output = (y_output * 255).astype(np.uint8).transpose(1, 2, 0)
                    ycrcb_input = (y_input * 255).astype(np.uint8).transpose(1, 2, 0)
                    ycrcb_target_patches.append(ycrcb_target)
                    ycrcb_output_patches.append(ycrcb_output)
                    ycrcb_input_patches.append(ycrcb_input)
                    patch_idx += 1

                    # 8x8 이미지들을 32x32로 합치기
                    if patch_idx % 16 == 0 and patch_idx > 0:
                        patch_idx = 0
                        combined_target_image = combine_8x8_patches_to_32x32(ycrcb_target_patches)
                        combined_output_image = combine_8x8_patches_to_32x32(ycrcb_output_patches)
                        combined_input_image = combine_8x8_patches_to_32x32(ycrcb_input_patches)
